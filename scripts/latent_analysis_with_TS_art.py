...
class StatesAnalysis(object):   
    ...
    def loadData(self):
        ...
        self.sleepStages = ['Artefacts', 'Wake', 'NREM', 'TS' 'REM']
    ...
    def analyzeStates(self):
        ...
        for i in self.uniqueStates[:, 0]:
            ...
            self.BoxPlotsSimple(dPlotEEG, './boxPlotsBackProjectedData/', len(idx), i, EEG_labels, length_artefacts,
                                        length_awake, length_nrem, length_TS, length_rem, EEG_range, id_bin)
    ...
    def groupStatistics(self):
        ...
        for lstate in self.lstatesIDs:
            ...
            length_artefacts = round((len(np.where((latent_frames[:, latent_frames.shape[1]-3]==0))[0])/float(len(latent_frames))),3)
            ...
            length_TS = round((len(np.where((latent_frames[:, latent_frames.shape[1]-3]==4))[0])/float(len(latent_frames))),3)
            ...
            fig.suptitle('Latent State ' + str(lstate) + '\nArtefacts: ' + str(round(length_artefacts,3)*100) + '%, Wakefulness: ' + str(round(length_awake,3)*100) + '%, NREM Sleep: ' + str(round(length_nrem,3)*100) + '\n TS Sleep: ' + str(round(length_TS,3)*100)+ '%, REM Sleep: ' + str(round(length_rem,3)*100) + '%' + '\nTotal number of epochs: ' + str(len(latent_frames)), fontsize=30, fontweight='bold')
            ...
            fig.suptitle('LS ' + str(lstate) + ' - Total: ' + str(len(latent_frames)) + ' epochs' + '\nARTEFACTS: ' + str(length_artefacts*100) + '%, WAKE: ' + str(length_awake*100) + '\nTS: ' + str(length_TS*100) + '%, NREM: ' + str(length_nrem*100) + '%, REM: ' + str(length_rem*100) + '%', fontsize=35, fontweight='bold')
    ...
    def stageDistribution(self):
        ...
        #-- Visualize array:
        column_labels = ['Artefacts', 'Wakefulness', 'NREM', 'TS', 'REM']
    ...
    def entropyMIcontrol(self):
        ...
        with open ('./entropyMI/mutualInformation.txt','a') as f:
            f.write("\n Stage Entropy:")
            f.write("\n Artefacts = %s" %stagesH[0])
            f.write("\n Wakefulness = %s" %stagesH[1])
            f.write("\n NREM = %s" %stagesH[2])
            f.write("\n TS = %s" %stagesH[3])
            f.write("\n REM = %s\n" %stagesH[4])
            f.write("\n Stage Normalized MI:")
            f.write("\n The MI/H(artefacts) = %s" %(MI_stage[0]/stagesH[0]))
            f.write("\n The MI/H(wake) = %s" %(MI_stage[1]/stagesH[1]))
            f.write("\n The MI/H(nrem) = %s" %(MI_stage[2]/stagesH[2]))
            f.write("\n The MI/H(TS) = %s" %(MI_stage[3]/stagesH[3]))
    ...
    def lstateStageDistribution(self, inputMat, columnState, columnStage):
        ...
        for i in lstates:
            ...
            length_artefacts = len(np.where(obsKeys[:, columnStage]==0)[0])        
            length_awake = len(np.where(obsKeys[:, columnStage]==1)[0])         
            length_nrem = len(np.where(obsKeys[:, columnStage]==3)[0])  
            length_TS = len(np.where(obsKeys[:, columnStage]==4)[0])          
            length_rem = len(np.where(obsKeys[:, columnStage]==5)[0])

            vec_len = (length_artefacts/lstatePopulation, length_awake/lstatePopulation, length_nrem/lstatePopulation, length_TS/lstatePopulation, length_rem/lstatePopulation)
            latent_color.append(vec_len)
            counts.append([length_artefacts, length_awake, length_nrem, length_TS, length_rem])
    ...
    def BoxPlotsSimple(self, d_to_plot_1, fig_path, population, i, labels_1, length_artefacts, length_awake, length_nrem, length_TS, length_rem, range_1, id_bin):
        '''
    ...
    def MI_stimulusH_barPlot(self, MI_stage, stagesH, overAll, saveDir):
        ...
        width = 0.5
        bar_width = 0.43
        ax1.bar(ind, MI_stage[0]/stagesH[0], bar_width, color=colors[5]['color'], edgecolor = "none", label="Artefacts")
        ax1.bar(ind+width, MI_stage[1]/stagesH[1], bar_width, color=colors[9]['color'], edgecolor = "none", label="Wakefulness")
        ax1.bar(ind+2*width, MI_stage[2]/stagesH[2], bar_width, color=colors[8]['color'], edgecolor = "none", label="NREM")
        ax1.bar(ind+3*width, MI_stage[3]/stagesH[3], bar_width, color=colors[3]['color'], edgecolor = "none", label="TS")
        ax1.bar(ind+4*width, MI_stage[4]/stagesH[4], bar_width, color=colors[1]['color'], edgecolor = "none", label="REM")
        ax1.bar(ind+0.5+5*width, overAll, bar_width, color='#40e0d0', edgecolor = "none", label="OverAll")
    ...
    def plotHistogram(self, saveDir, name, yLim):       
        ...
        white_patch = mpatches.Patch(color=(1.0, 1.0, 1.0), label='Artefacts')
        red_patch = mpatches.Patch(color=(0.0, 0.0, 1.0), label='Wakefulness')
        green_patch = mpatches.Patch(color=(0.0, 1.0, 0.0), label='NREM')
        grey_patch = mpatches.Patch(color=(0.5, 0.5, 0.5), label='TS')
        blue_patch = mpatches.Patch(color=(1.0, 0.0, 0.0), label='REM')
        

        legend_properties = {'weight':'bold'}
        legend = plt.legend(handles=[white_patch, red_patch, green_patch, grey_patch, blue_patch], borderaxespad=0., fontsize=30, prop=legend_properties)
